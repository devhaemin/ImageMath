...
extension Assignment{
    static func getAssignmentDummyData() -> [Assignment]{
        return [Assignment(userSeq: 0, submitState: 0, assignmentSeq: 0, title: "2주차", contents: ".", postTime: 100, endTime: 100, lectureTime: 100, lectureName: "고3 fly 모의고사 나형 대치이강")]
    }
}
extension Assignment{
...
extension Assignment{
    static func getAssignmentList(completion: @escaping (Result<Assignment, Error>) -> Void){
        let con = AssignmentRouter.getAssignment(nil)
        APIClient.perform(con, completion: completion)
    }
}
