...
public class AssignmentEditPresenter implements AssignmentEditContract.AssignmentEditPresenter {

    private static final String TAG = "AssignmentEditPresenter";
    AssignmentEditContract.AssignmentEditView view;

    public AssignmentEditPresenter(AssignmentEditContract.AssignmentEditView view) {
        this.view = view;
    }


    @Override
    public void updateAssignmentData(String assignmentSeq) {
        GlobalApplication.getAPIService().getAssignmentInfo(GlobalApplication.getAccessToken(),assignmentSeq).enqueue(new Callback<Assignment>() {
            ...
            public void onResponse(Call<Assignment> call, Response<Assignment> response) {
                if(response.code() == 200 && response.body() != null){
                    view.showAssignmentData(response.body());
                }else{
                    Log.e("AssignmentEditPresenter",response.message());
    ...
    public void uploadAssignment(HashMap<String, String> assignmentMap, ArrayList<ServerFile> answerFiles) {
        GlobalApplication.getAPIService().editAssignmentInfo(GlobalApplication.getAccessToken(),assignmentMap).enqueue(new Callback<Assignment>() {
            @Override
            public void onResponse(Call<Assignment> call, Response<Assignment> response) {
                if(response.code() == 200 && response.body() != null){
                    ...
                    view.showSuccess("과제 수정이 성공적으로 완료되었습니다.");
                }else{
                    view.showToast("과제 수정이 실패했습니다.");
